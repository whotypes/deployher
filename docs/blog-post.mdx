...
For **local development**, run the API with `bun run dev` (Bun) and the Vite dev server in another terminal (`bun run dev:vite`); Vite **proxies** `/api`, `/assets`, and preview paths to the Bun backend so the browser stays on a single origin (see `vite.config.ts` and `VITE_DEV_API_URL`).
