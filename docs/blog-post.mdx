...
That's where Deployher started. Not from some grand vision of disrupting the PaaS industry. Just from a Sunday morning brain worm that wouldn't go away.
...
Deployher implements all seven. On a single VPS. With a single runtime. In about 4,000 lines of TypeScript.
...
The thing that really sold me was the S3 client. Deployher stores every build artifact in S3-compatible storage. In a Node.js world, that means installing `@aws-sdk/client-s3`, wrestling with the 47 packages it drags in, and dealing with the SDK's baroque API. In Bun:
...
No `ioredis`. No `redis` package. No connection pooling library. Just raw Redis commands over a fast connection. And critically for Deployher, it supports `EVAL` for Lua scripts, which powers the entire concurrency control system (more on that later).

<Rabbit>
I actually benchmarked Bun's Redis client against ioredis early on. For the `XADD`/`XREADGROUP` workload Deployher uses, Bun was roughly 2-3x faster in throughput. Not that it matters at Deployher's scale—but it's nice to know you're not leaving performance on the table.
</Rabbit>
...
Seven dependencies. For a full deployment platform with auth, database, container orchestration, and a React dashboard. Compare that to a typical Next.js `node_modules` and weep.
...
On boot, Deployher runs `Bun.build()` to bundle the client-side React assets (CSS, JS) into `/dist/client`. Then it starts the HTTP server. Then it background-checks S3 connectivity. That's the whole lifecycle.
...
There's also a dual API pattern—HTML shells (SPA `index.html` or streamed bodies) and JSON under `/api/`. The router checks the request's `Accept` header via `wantsHtml()` to decide whether to return HTML or a 404 JSON response where that split still applies.
...
But the really clever part is the **account-level concurrency control**. Without it, one user could flood the queue with 50 deployments and starve everyone else. Deployher uses a pair of Lua scripts to atomically manage per-account build slots:
...
deployment-worker:
  ...
  volumes:
    ...
    - /tmp/deployher-builds:/tmp/deployher-builds
```
...
**3. Container cleanup.** Before starting, the worker prunes any stale build containers from previous runs. Every container Deployher creates gets labeled with `io.deployher.build=true` and `io.deployher.deployment={id}`, so cleanup is a simple Docker API filter:
...
const runDockerCommand = async (cmd, options) => {
  ...
  const container = await dockerClient.createContainer({
    ...
    Labels: {
      "io.deployher.build": "true",
      "io.deployher.deployment": options.deploymentId
    },
    HostConfig: {
...
The image selection is automatic: `bun` commands get `deployher-bun-build-image:latest`, Python commands get `python:3.12-bookworm`, everything else gets `node:22-bookworm` or `deployher-node-build-image:latest` when configured. The Bun builder image layers Python and common native build libraries on top of Bun so packages like `canvas` can compile during `bun install`. Images are pulled on demand and cached in a `Map` so the same image isn't pulled twice across concurrent builds.

The bind mount (`options.cwd:/workspace`) is why the worker and the Docker daemon need to share a filesystem path. In Docker Compose, both `/tmp/deployher-builds` on the host and inside the worker container point to the same physical directory.
...
In Deployher's `docker-compose.yml`, Garage runs as a single container:
...
### Garage bootstrap: the deployher CLI

The **`deployher`** CLI (under `cli/`) bootstraps the storage layer and app stack. **It does not require Bun on the host** for migrate/seed — those run inside **`oven/bun`** via `docker run` (override with **`BUN_IMAGE`**). You can run it with **`bun run deployher start`** or, after **`bun run build:cli`**, **`./dist/deployher-cli start`**. **`deployher start`** will:
...
**`deployher`** also supports a Nexus container registry for environments where you want to cache Docker images locally (conceptually similar to):
...
After mirroring the upstream bases, **`deployher start`** also builds and pushes `deployher-node-build-image:latest` and `deployher-bun-build-image:latest` so workers can use the same preconfigured build images in local and remote environments.
...
## The UI: Vite SPA + Bun JSON APIs

The dashboard is a **single-page application** built with **Vite**, **React**, and **React Router**. The Bun HTTP server does **not** import or execute React: it serves the built shell from `dist/client`, streams **non-HTML** bodies where needed (build logs, Docker, storage), and exposes **JSON** under `/api/*` (including aggregate UI payloads such as `/api/workspace/dashboard` and `/api/ui/...`).

Browser navigations receive the same `index.html` shell (with optional CSRF meta injection); the client loads route chunks and fetches data with `fetch(..., { credentials: "include" })`. Styles ship as hashed CSS under `/assets/*` from `vite build`.

For **local development**, run the API with `bun run dev` (Bun) and the Vite dev server in another terminal (`bun run dev:vite`); Vite **proxies** `/api`, `/d`, and `/preview` to the Bun backend while hashed assets stay on the Vite origin (see `vite.config.ts` and `VITE_DEV_API_URL`). The static marketing site lives under `apps/marketing` (`bun run dev:marketing`).

<Rabbit>
Keeping React off the server keeps the Bun binary small, avoids streaming HTML from React, and makes caching and CSRF boundaries predictable. The tradeoff is first paint after load—mitigated with loading states and the same API the UI already needs for interactivity.
</Rabbit>
...
The `node-build-image` service is an init container—it builds a pre-configured Node.js image with common tools installed, tags it as `deployher-node-build-image:latest`, prints "node builder image ready," and exits. The deployment workers reference this image for Node.js builds instead of pulling `node:22-bookworm` fresh every time.

The `bun-build-image` service does the same for Bun-based projects. It produces `deployher-bun-build-image:latest`, which includes Python plus the Cairo and Pango development libraries commonly needed by native Bun dependencies such as `canvas`.
...
4. Run **`deployher start`** (from a machine with Bun, or use a compiled **`deployher-cli`** binary)
...
The only gotcha is the shared filesystem. When the worker creates a temp directory at `/tmp/deployher-builds/build-abc123`, the Docker daemon needs to see that same path. In Compose, you just volume-mount `/tmp/deployher-builds` into both the worker and the host.
...
### Static marketing + SPA dashboard is a fine split

The apex site is a small **Astro** build with React islands for the landing copy; the product UI is a **Vite** SPA on the dash host. Same design system imports, but nginx serves marketing HTML without touching the Bun API process, and the dashboard shell stays one `index.html` plus route-level chunks.
...
Deployher is functional but not finished. The roadmap includes:
