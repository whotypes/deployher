...
export const injectS3IntoEnvFile = async (
  ...
  for (const line of lines) {
    if (line.startsWith("S3_BUCKET=")) {
      ...
      hasBucket = true;
    } else if (line.startsWith("S3_AVATAR_BUCKET=")) {
      out.push(`S3_AVATAR_BUCKET=${avatarBucket}`);
      hasAvatarBucket = true;
  ...
  if (!hasAvatarBucket) out.push(`S3_AVATAR_BUCKET=${avatarBucket}`);
